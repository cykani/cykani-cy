...
const deploymentProgress = Math.round((profilesDeployed / profilesTarget) * 100);
const deploymentBarCount = 42;
const activeDeploymentBars = Math.round((profilesDeployed / profilesTarget) * deploymentBarCount);

const deploymentBars = Array.from({ length: deploymentBarCount }, (_, index) => ({
  id: `deployment-${index + 1}`,
  active: index < activeDeploymentBars,
}));
  ...
  return (
    <section className="grid grid-cols-1 gap-4 xl:grid-cols-12">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <div className="truncate font-medium text-primary-foreground text-xs leading-none">
                      Proxy rotation — EU cluster
                    </div>
                    <div className="truncate text-[10px] text-primary-foreground/75">Frankfurt, Germany</div>
                  </div>
                </div>
              </div>
      ...
      <Card className="xl:col-span-4">
        <CardHeader>
          <CardTitle>Weekly Profile Target</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-col gap-1">
          <div className="flex items-end justify-between gap-3">
            <div className="font-medium text-2xl tabular-nums leading-none">
              {profilesDeployed} <span className="font-normal text-base text-muted-foreground">deployed</span>
            </div>
            <div className="text-muted-foreground text-sm tabular-nums">{profilesTarget} target</div>
          </div>
          <div className="flex h-10 w-full items-end gap-0.5">
            {deploymentBars.map((bar) => (
          ...
          <p className="text-muted-foreground text-sm">
            {deploymentProgress}% of this week&apos;s profile deployment target reached.
          </p>
        </CardContent>
      </Card>
    </section>
